...
export type ChelCoachSnapshot = {
  report: GameReport;
  /** ReportContext only ever carries the built-in sample; real analyses render from /analysis/:id/report. */
  source: "sample";
  isPremium: boolean;
  hasAnalysis: boolean;
};
...
export function buildChelCoachTools(
  ...
  return [
    defineReadOnlyTool({
      ...
      description:
        "Report whether this session has viewed the sample gameplay analysis and whether the sample film room is unlocked. Every report these tools expose is ChelCoach's built-in sample, never a real analysis of the user's gameplay. Call this first.",
      read() {
        ...
        return {
          ...
          isPremium,
          reportSource: source,
          note: "This is ChelCoach's built-in sample report, not a real analysis of the user's gameplay.",
        };
      },
    }),
